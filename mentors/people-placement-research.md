# Origin40 People Placement Research

**Purpose:** classify confirmed and proposed people into the right operational category: Program
Leads, Founder Mentors, or Featured Speakers.

## Placement Rules

| Category | Meaning | cPanel location |
|---|---|---|
| Program Leads | Runs the program, live rooms, reviews, Discord check-ins, build support, and founder operations | Program Leads |
| Founder Mentors | Can be matched to founders for repeated support, office hours, review, or follow-up | Founder Mentors |
| Featured Speakers | Takes a one-off 1-2 hour masterclass/keynote/clinic and leaves follow-up to the program leads | Featured Speakers |

## Final Placement

| Person | Placement | Role / track | Status | Reason |
|---|---|---|---|---|
| David Onilude | Program Leads | Lead Facilitator / Program Lead | Confirmed | Public profile fits digital literacy, software/product building, Beere Software, Tech Now Global, and program leadership. |
| Kazeem Quadri | Program Leads | Co-Facilitator / Build Lead | Confirmed | Public profile fits software engineering, full-stack systems, Beere Software, Tech Now Global, and build support. |
| Dotonu Wonder | Program Leads | Beere program operations / founder support | Confirmed | Confirmed by program owner as part of Beere and a Program Lead for Origin40 delivery, coordination, and founder operations. |
| Avoseh Yohan | Program Leads | Beere program operations / founder support | Confirmed | Confirmed by program owner as part of Beere and a Program Lead for Origin40 delivery, coordination, and founder operations. |
| Moses Deborah | Program Leads | Beere program operations / founder support | Confirmed | Confirmed by program owner as part of Beere and a Program Lead for Origin40 delivery, coordination, and founder operations. |
| Bukola Aladesulu | Founder Mentor | Marketing / positioning / opportunity readiness / founder communication | Confirmed | Confirmed by the programme owner for marketing as well as founder communication. Best placed for audience definition, messaging, practical go-to-market planning, positioning, opportunity readiness, and public communication. |
| Victory Ashaka | Founder Mentor | Founder storytelling / Demo Day delivery | Confirmed | Public profile fits youth leadership, spoken word, climate/social impact, community momentum, and founder storytelling. Program owner confirmed Victory belongs in the mentor pool. |
| Damilola Obaro | Founder Mentor | Legal/Operations | Confirmed | Bio and public law-firm profile fit startup law, corporate/commercial law, compliance, IP, data privacy, contracts, and fundraising readiness. |
| Stanley Anigbogu | Founder Mentor | Technical/Product/Impact | Confirmed | Public profiles fit LightEd, clean energy, technical innovation, circular design, community adoption, and impact product support. |
| Erioluwa Adeyinka | Founder Mentor | Education / social impact / operations | Confirmed | Bio provided by program owner. Fits education founders, social-impact ventures, development-sector partnerships, NGO/government collaboration, and operational execution. |
| Dr. Chukwudi Ezenwa Akubueze | Founder Mentor | Healthtech / public health / clinical operations | Invited | Bio provided, but participation is not yet confirmed. Do not place on the live calendar yet. |
| Mofiyinfoluwa Faniyi | Founder Mentor / Guest Speaker | Growth / partnerships / marketing / investor relations | Confirmed | Confirmed by program owner as mentor and guest speaker. Place on Week 3 growth, partnerships, and go-to-market session, with additional build-lab and final messaging support. |
| David Oke Opeyemi | Featured Speakers | Legacy keynote, trade, policy, youth development | Confirmed | Confirmed by program owner as honoree/celebrant; public profile fits trade governance, international development, policy, and youth development. |
| Samuel Afolabi | Featured Speakers | Founder execution / local market building | Invited | Public profile likely fits MyAgent/founder-market fit; identity should be confirmed before publishing external bio. |
| Gbemi Adunbarin | Featured Speakers | E-commerce, AI commerce, product trust | Invited | Public event/social references fit Vegeel, commerce, AI, and trust. Confirm preferred bio before publishing. |
| Dr. Abiola Olamoyegun | Featured Speakers | Cybersecurity, IT assurance, governance, risk | Invited | Public profiles list PHEAA IT/security leadership and cybersecurity speaker placement. |
| Prof. Carlos Azevedo | Featured Speakers | Social innovation, impact, venture story | Invited | Public institutional profile fits Yunus Social Innovation Center, Católica Lisbon, social innovation, and impact strategy. |
| Marcene Mitchell | Featured Speakers | Climate finance / sustainable business / investment readiness | Confirmed | Confirmed by program owner. Official and IFC sources verify extensive leadership in climate finance, sustainable business, clean energy, nature-based solutions, and emerging-market investment. Best placed in Week 4. |
| Segun Ajanaku | Founder Mentor | UI/UX / product design / branding | Confirmed | Confirmed by program owner as a UI/UX and branding mentor. Best placed in Week 2 for user flows, wireframes, prototyping, usability testing, visual identity, and coherent product presentation, with a final brand review before the showcase. Public profile still to be supplied or verified. |

## Source Notes

| Person | Source |
|---|---|
| David Onilude | `https://davidonilude.com.ng/` |
| Kazeem Quadri | `https://kazeemquadri.com/` |
| Bukola Aladesulu | `https://www.coachli.co/bukolaaladesulu` and `https://about.me/bukolaaladesulu` |
| Victory Ashaka | `https://victoryashaka.com/` |
| Damilola Obaro | `https://www.ul-law.com/team_member/tomilola-opaleye/` |
| Stanley Anigbogu | `https://thecommonwealth.org/young-nigerian-innovator-lighting-and-powering-communities` |
| David Oke Opeyemi | `https://www.wtca.org/users/118-david-opeyemi-oke` |
| Samuel Afolabi | `https://ng.linkedin.com/in/samuel-afolabi-4a546787` |
| Gbemi Adunbarin | `https://www.instagram.com/reel/C66GZpaooGq/` |
| Dr. Abiola Olamoyegun | `https://ciso-chicago.coriniumintelligence.com/speakers` |
| Prof. Carlos Azevedo | `https://knowledgecentre.euclidnetwork.eu/authors-and-experts/carlos-azevedo/` |

## Bio Gaps To Request

| Person | Needed |
|---|---|
| Gift Best | Short bio, current role/org, strongest support area, preferred public link |
| Segun Ajanaku | Short bio, current role/org, portfolio or preferred public profile |
| Samuel Afolabi | Confirmation of exact intended public profile and preferred bio |
| Gbemi Adunbarin | Preferred public bio and session availability |
